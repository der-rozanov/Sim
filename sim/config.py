...
class AircraftParams:
    ...
    # --- Масса и инерция ---
    mass: float = 10     # кг                            [АВТОР]
    Jy:   float = 2.27    # кг·м², момент инерции тангажа [АВТОР]

    # --- Геометрия крыла ---
    S:    float = 0.55      # м², площадь крыла             [АВТОР]
    b:    float = 2    # м, размах                     [АВТОР]
    c:    float = 0.2   # м, средняя хорда              [АВТОР]
    ...
    CL0:  float = 0.1      # при alpha=0, q=0, delta_e=0
    ...
    e_oswald: float = 0.2   # КПД Освальда
    ...
    M_sigmoid: float = 30.0   # крутизна перехода
    alpha_stall: float = 0.4712  # рад ≈ 27°, угол срыва потока

    # --- Силовая установка ---
    k_motor:  float = 80.0   # коэффициент двигателя        [АВТОР]
    S_prop:   float = 0.1027  # м², площадь диска винта
    C_prop:   float = 1.0     # коэффициент тяги
    T_max:    float = 40.0   # Н, физический предел тяги    [АВТОР: уточнить]
